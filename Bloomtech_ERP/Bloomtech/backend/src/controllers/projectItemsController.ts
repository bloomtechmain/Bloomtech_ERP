...
export const updateItem = async (req: Request, res: Response) => {
  ...
  try {
    await pool.query('BEGIN')
    const existing = await pool.query('SELECT unit_cost FROM project_items WHERE project_id=$1 AND requirements=$2', [projectId, requirements])
    if (!existing.rows.length) {
      await pool.query('ROLLBACK')
      return res.status(404).json({ error: 'item_not_found' })
    }
    const oldCost = Number(existing.rows[0].unit_cost || 0)
    const delta = Number(unit_cost) - oldCost
